...
- 服务器依赖：Docker、Docker Compose、Nginx、rsync；部署用户必须能创建并写入 `DEPLOY_PATH`，workflow 不会回退到其他目录。部署用户仍需要能执行 `sudo nginx -t` 与 `sudo systemctl reload nginx`。
...
- `http://47.93.150.173/operator/plans`

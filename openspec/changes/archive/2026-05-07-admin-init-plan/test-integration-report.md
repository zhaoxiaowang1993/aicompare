...
- `POST /api/admin/plans` 不创建未导入计划，返回 `400 PLAN_IMPORT_REQUIRED`。
...
- closed 计划拒绝 owner 修改、CSV 导入，禁止 operator 查看/操作任务与标注写入。
...
| 409 错误码 | 通过：closed 计划导入、operator 任务操作/写入与非法 closed 编辑返回冲突类错误 |
